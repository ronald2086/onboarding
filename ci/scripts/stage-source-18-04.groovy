...
void setTagAndVersion(String artifactBuildNumber) {
    ...
    env.tagId = "ems-1804-${env.ENVIRONMENT}-build-${artifactBuildNumber}"
    env.artifactVersion = "ems-1804-${env.ENVIRONMENT}-build-${artifactBuildNumber}-SNAPSHOT"
}
...
def processBacnetProcess() {
    echo "========Process BAcnet========="
    container('docker-manage-ubuntu18-04') {
        withCredentials([sshUserPrivateKey(credentialsId: 'jenkins-bitbucket-ssh', keyFileVariable: 'KEY')]) {
            sh '''
        mkdir -p ~/.ssh
        cat "${KEY}" > ~/.ssh/id_rsa
...
def buildCode() {
    ...
    container('docker-manage-ubuntu18-04') {
        configFileProvider([configFile(fileId: 'maven-settings', variable: 'MAVEN_SETTINGS_XML')]) {
          ...
          cp ci/scripts/debian_creation.sh .
          cp ci/scripts/cloud_debian.sh .
          cp ci/scripts/adr_debian2.sh .
          chmod +x ./*.sh
          bash ./debian_creation.sh a64 ${UBUNTU_RELEASE}

          bash ./cloud_debian.sh
          bash ./adr_debian2.sh
        """
            //  cp artifacts/scripts/1804_Ubuntu_Upgrade/package18.04ISO.sh .
            //  chmod +x ./*.sh
            //  bash ./package18.04ISO.sh "/mnt/data/home/jenkins"
        }
    }
}

def getBaseIsoFromNexus() {
    container('docker-manage-ubuntu18-04') {
        configFileProvider([configFile(fileId: 'maven-settings', variable: 'MAVEN_SETTINGS_XML')]) {
            echo "==========Downloading base ISO from nexus=========="
            sh '''
          mkdir -p EM/base-iso
          mvn dependency:get -DrepositoryId=maven-releases -Dartifact=com.enlightedinc:manage-app-base-iso:1.0.0:iso -Dtransitive=false -Ddest=EM/base-iso -DremoteRepositories=https://nexus.aws.enlightedinc.com/repository/maven-releases/ -s $MAVEN_SETTINGS_XML
          chmod -R 777 EM/base-iso
          '''
            echo "==================Successfully downloaded base ISO=================="
        }
    }
}

def buildISO() {
    echo "===============BUILD AND PACKAGE ISO=============="
    container('docker-manage-ubuntu18-04') {
        getBaseIsoFromNexus()
        sh '''
      cd EM
      cp ci/scripts/package18.04ISO.sh .
      chmod +x ./*.sh
      bash ./package18.04ISO.sh "base-iso"
    '''
    }
}

def getMavenRepoAndVersionForNexus(String buildNumber) {
    echo "========Get correct maven repo and artifact version========"
    if (env.BUILD_GIT_BRANCH.startsWith('release/')) {
        env.mavenRepo = 'maven-releases'
        env.artifactVersion = "${POM_VERSION}.${buildNumber}"
    } else {
        env.mavenRepo = 'maven-snapshots'
        env.artifactVersion = "${POM_VERSION}.${buildNumber}-SNAPSHOT"
    }
}

def uploadDebianAllToNexus() {
    echo "========Uploading deb files to Nexus========"
    script {
        CI_ERROR = "Upload deb files failed"
    }
    if (env.DEBUG_MODE == 'false') {
        container('docker-manage-ubuntu18-04') {
            configFileProvider([configFile(fileId: 'maven-settings', variable: 'MAVEN_SETTINGS_XML')]) {
                getMavenRepoAndVersionForNexus("${BUILD_NUMBER}")

                echo "==========Upload deb files=========="
                sh '''
			manageAppVersion=$(mvn help:evaluate -Dexpression=currentVersion -q -DforceStdout)
            if [[ ! -f $(find "EM/debian_em_all2" -name '*_a64_em_all.deb') ]]
            then
              echo "Required nexus deb file does not exist, please check build pipeline step"
              exit 1
            fi

            mvn deploy:deploy-file -DgroupId=com.enlightedinc -DartifactId=manage -DgeneratePom=true -Dpackaging=deb -DrepositoryId=${mavenRepo} -Dversion=${artifactVersion} -Durl=https://nexus.aws.enlightedinc.com/repository/${mavenRepo}/ -Dfile=`find EM/debian_em_all2 -name *_a64_em_all.deb | head -n 1` -DskipTests -s $MAVEN_SETTINGS_XML
        '''
            }
        } //container('docker-manage-ubuntu18-04')
    } else {
