locals {
  ...
  owner         = "${var.tenant_instance_name}@enlightedinc.com"
  root_domain   = "manage.enlightedinc.com"
  environment   = var.environment
  region        = var.region
  dr_region     = var.dr_region
  manage_domain = local.environment == "prod" ? "${local.service}.${local.root_domain}" : "${local.service}-${var.environment}-${var.region}.${local.root_domain}"
  ...
  # Just set feature flag to true in playground. Can be modified in AWS console if required
  feature_flag_use_republished_topics = true

  # Replace actual Manage application zip file below.
  manage_application_bundle = var.manage_application_bundle == "" ? "${path.module}/manage-draft-app-bundle-2021.zip" : var.manage_application_bundle
  ...
  pagerduty_endpoint  = "https://events.pagerduty.com/integration/1e14bc1c7eb44108d097e70d60918918/enqueue"
}
...
#unique Manage UUID to be generated and used for the lifecycle of terraform state.
...
module "rds" {
  ...
  rds_map_server_id              = var.rds_map_server_id
  allowed_cidr_blocks            = [local.vpn_cidr, local.fargate_cidr]
  tenant_rds_snapshot_identifier = var.tenant_rds_snapshot_identifier
...
module "create-domain-cert" {
  source                 = "../base/create-domain-cert"
  service                = local.service
  region                 = local.region
  environment            = local.environment
  owner                  = local.owner
...
module "pca-cert" {
  ...
  owner                  = local.owner
  default_map_server_id  = local.default_map_server_id
  manage_uuid            = random_uuid.manage_uuid.id
  network_account_number = local.network_account_number
...
output "manage_uuid" {
  ...
  value       = random_uuid.manage_uuid.id
}
